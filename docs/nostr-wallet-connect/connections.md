...
Tap **Create Connection** when you're done. ZEUS shows a QR code and a `nostr+walletconnect://` link — scan or paste that into the other app to finish pairing.
...
After **Create Connection**, ZEUS shows a QR code and link — scan or paste that into the other app. The first request can take a few seconds. If a timeout appears on the pairing screen, you can close it; the connection often works once the other app tries again. Make sure you're using the latest QR code, not an old one.
